...
/**
 * Interactive dot-grid background for the landing page only.
 *
 * Three things happen on a single fixed canvas:
 *  - a breathing grid of dots that brighten near the cursor,
 *  - ambient "constellation" lines from the cursor to nearby dots,
 *  - click-to-connect: each click snaps to the nearest dot and draws a
 *    glowing line from the previous one (right-click ends the chain).
 *
 * Layering: this sits at z-0, *below* the animated GrainOverlay (z-1) so the
 * existing film grain still textures everything, and below page content (z-2+)
 * so text and links render on top. The canvas listens for clicks on itself
 * only; real links above it (pointer-events:auto) win the hit-test, so
 * navigating never draws a dot. See HomePage where `main` is pointer-events-none
 * and each Link opts back in.
 */
